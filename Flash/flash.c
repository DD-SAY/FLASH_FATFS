...
{
    uint16_t pageRemain = 256;      // W25Qxx每个页命令最大写入字节数:256字节;    
		uint8_t cmd = PAGE_PROGRAM_CMD;
...
//        
//            HAL_GPIO_WritePin(W25Q64_CHIP_SELECT_PORT, W25Q64_CHIP_SELECT_PIN, GPIO_PIN_RESET);                     // 低电平,开始
//            SPI_Transmit(&cmd, 1);              // 命令: page program : 02h , 每个写页命令最大缓存256字节
////            cW25qx_SendByte((uint8_t )(addr>>16));    // 地址
////            cW25qx_SendByte((uint8_t)(addr>> 8));
////            cW25qx_SendByte ((uint8_t)addr); 
//						SPI_Transmit((uint8_t*)(addr>>16), 1);
//						SPI_Transmit((uint8_t*)(addr>>8), 1);
//						SPI_Transmit((uint8_t*)(addr), 1);
//            for(uint16_t i=0;i<pageRemain; i++)      // 发送写入的数据 
//                SPI_Transmit( &p[i] ,1);        // 高电平, 结束
//           HAL_GPIO_WritePin(W25Q64_CHIP_SELECT_PORT, W25Q64_CHIP_SELECT_PIN, GPIO_PIN_SET);     
//        
//        W25QXX_Wait_Busy ();                    // 等待空闲    
//      
...
//        addr = addr + pageRemain ;              // 写地址增加一页字节数
			W25QXX_Page_Program(p,addr,256);
    }      
}
